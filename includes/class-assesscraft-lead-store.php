...
final class AssessCraft_Lead_Store {
	...
	public function render(): void {
		...
		<div class="wrap ac-leads-page">
			...
			<div class="ac-leads-hero"><div><span class="ac-eyebrow"><?php esc_html_e( 'Lead management', 'assesscraft' ); ?></span><h2><?php esc_html_e( 'Review and manage consultation interest', 'assesscraft' ); ?></h2><p><?php esc_html_e( 'Search stored requests, review result context, and manage data safely from one place.', 'assesscraft' ); ?></p></div><div class="ac-leads-summary"><article><strong><?php echo absint( $total ); ?></strong><span><?php esc_html_e( 'Matching requests', 'assesscraft' ); ?></span></article><article><strong><?php echo absint( get_option( self::RETENTION_OPTION, 90 ) ); ?></strong><span><?php esc_html_e( 'Retention days', 'assesscraft' ); ?></span></article></div></div>
			...
			<?php foreach ( $rows as $row ) : $delete_url = wp_nonce_url( admin_url( 'admin-post.php?action=assesscraft_delete_lead&lead=' . absint( $row->id ) ), 'assesscraft_delete_lead_' . absint( $row->id ) ); ?><tr><td><time><?php echo esc_html( get_date_from_gmt( $row->created_at, 'M j, Y' ) ); ?></time><small><?php echo esc_html( get_date_from_gmt( $row->created_at, 'H:i' ) ); ?></small></td><td><strong><?php echo esc_html( $row->contact_name ); ?></strong><br><a href="mailto:<?php echo esc_attr( $row->email ); ?>"><?php echo esc_html( $row->email ); ?></a><?php if ( $row->company ) : ?><br><small><?php echo esc_html( $row->company ); ?></small><?php endif; ?><details><summary><?php esc_html_e( 'Contact details', 'assesscraft' ); ?></summary><p><?php echo esc_html( $row->phone ); ?></p><p><?php echo nl2br( esc_html( $row->message ) ); ?></p></details></td><td><strong><?php echo esc_html( $row->assessment_name ); ?></strong></td><td><span class="ac-score-pill"><?php echo esc_html( round( (float) $row->overall_score ) ); ?>%</span><small><?php echo esc_html( $row->classification ); ?></small></td><td><span class="ac-profile-pill"><?php echo esc_html( $row->profile ?: '—' ); ?></span></td><td><button type="button" class="button-link-delete ac-confirm-delete" data-delete-url="<?php echo esc_url( $delete_url ); ?>" data-confirm-phrase="DELETE" data-delete-label="<?php esc_attr_e( 'Delete request', 'assesscraft' ); ?>"><?php esc_html_e( 'Delete', 'assesscraft' ); ?></button></td></tr><?php endforeach; ?>
			</tbody></table></div>
			<?php
			echo wp_kses_post(
				paginate_links(
			...
			?>
			<section class="ac-lead-privacy-settings"><div><span class="dashicons dashicons-shield" aria-hidden="true"></span><div><h2><?php esc_html_e( 'Privacy and retention', 'assesscraft' ); ?></h2><p><?php esc_html_e( 'Storage is disabled by default. Retention cleanup runs daily through WordPress Cron.', 'assesscraft' ); ?></p></div></div><div class="ac-lead-privacy-actions"><?php if ( AssessCraft_Features::available( 'lead_csv_export' ) ) : ?><form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><input type="hidden" name="action" value="assesscraft_save_lead_settings"><?php wp_nonce_field( 'assesscraft_save_lead_settings' ); ?><label><?php esc_html_e( 'Automatically delete after', 'assesscraft' ); ?> <select name="retention_days"><?php foreach ( array( 30 => '30 days', 90 => '90 days', 180 => '180 days', 365 => '365 days', 0 => 'Keep forever' ) as $days => $label ) : ?><option value="<?php echo absint( $days ); ?>" <?php selected( (int) get_option( self::RETENTION_OPTION, 90 ), $days ); ?>><?php echo esc_html( $label ); ?></option><?php endforeach; ?></select></label><button class="button button-primary"><?php esc_html_e( 'Save retention', 'assesscraft' ); ?></button></form><?php else : ?><p class="ac-pro-note"><?php esc_html_e( 'Custom retention schedules — Pro Coming Soon. Free uses the 90-day privacy-safe default.', 'assesscraft' ); ?></p><?php endif; ?><form id="ac-purge-leads-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><input type="hidden" name="action" value="assesscraft_purge_leads"><?php wp_nonce_field( 'assesscraft_purge_leads' ); ?><button type="button" class="button ac-danger-button ac-confirm-delete" data-form-id="ac-purge-leads-form" data-confirm-phrase="DELETE ALL" data-delete-label="<?php esc_attr_e( 'Delete all requests', 'assesscraft' ); ?>"><?php esc_html_e( 'Delete all stored requests', 'assesscraft' ); ?></button></form></div></section>
			...
			<dialog class="ac-delete-dialog" id="ac-delete-dialog" aria-labelledby="ac-delete-dialog-title">
				<form method="dialog" class="ac-delete-dialog-card">
					<div class="ac-delete-dialog-icon"><span class="dashicons dashicons-warning" aria-hidden="true"></span></div>
					<h2 id="ac-delete-dialog-title"><?php esc_html_e( 'Confirm permanent deletion', 'assesscraft' ); ?></h2>
					<p id="ac-delete-dialog-copy"><?php esc_html_e( 'This action cannot be undone and the stored request cannot be recovered.', 'assesscraft' ); ?></p>
					<label><span><?php esc_html_e( 'Type', 'assesscraft' ); ?> <strong id="ac-delete-dialog-phrase">DELETE</strong> <?php esc_html_e( 'to continue', 'assesscraft' ); ?></span><input id="ac-delete-dialog-input" type="text" autocomplete="off" spellcheck="false"></label>
					<div class="ac-delete-dialog-actions"><button class="button" value="cancel"><?php esc_html_e( 'Cancel', 'assesscraft' ); ?></button><button type="button" class="button ac-danger-button" id="ac-delete-dialog-submit" disabled><?php esc_html_e( 'Delete permanently', 'assesscraft' ); ?></button></div>
				</form>
			</dialog>
		</div>
		<script>
		(function(){
			const dialog=document.getElementById('ac-delete-dialog'),input=document.getElementById('ac-delete-dialog-input'),phrase=document.getElementById('ac-delete-dialog-phrase'),submit=document.getElementById('ac-delete-dialog-submit'),title=document.getElementById('ac-delete-dialog-title');
			let target=null,required='';
			document.querySelectorAll('.ac-confirm-delete').forEach(function(button){button.addEventListener('click',function(){target=button;required=button.dataset.confirmPhrase||'DELETE';phrase.textContent=required;title.textContent=button.dataset.deleteLabel||'Confirm permanent deletion';input.value='';submit.disabled=true;dialog.showModal();setTimeout(function(){input.focus();},0);});});
			input.addEventListener('input',function(){submit.disabled=input.value.trim()!==required;});
			submit.addEventListener('click',function(){if(input.value.trim()!==required||!target)return;if(target.dataset.formId){document.getElementById(target.dataset.formId).submit();}else if(target.dataset.deleteUrl){window.location.assign(target.dataset.deleteUrl);}});
			dialog.addEventListener('close',function(){target=null;input.value='';submit.disabled=true;});
		}());
		</script>
		<?php
	}
	...
	public function delete_lead(): void {
		// guard_action() verifies the lead-specific nonce before deletion.
	...
	public function purge_leads(): void { $this->guard_action( 'assesscraft_purge_leads' ); global $wpdb; $wpdb->query( 'DELETE FROM ' . self::table() ); $this->redirect( 'purged' ); } // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	// guard_action() verifies the settings nonce before posted values are read.
	// phpcs:disable WordPress.Security.NonceVerification.Missing
	...
	// phpcs:enable WordPress.Security.NonceVerification.Missing
	public function cleanup_expired(): void { $days = absint( get_option( self::RETENTION_OPTION, 90 ) ); if ( ! $days ) return; global $wpdb; $cutoff = gmdate( 'Y-m-d H:i:s', time() - $days * DAY_IN_SECONDS ); $wpdb->query( $wpdb->prepare( 'DELETE FROM ' . self::table() . ' WHERE created_at < %s', $cutoff ) ); } // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	private function query( array $filters, int $limit, int $offset ): array { global $wpdb; $values = array(); $where = $this->where( $filters, $values ); $sql = 'SELECT * FROM ' . self::table() . ' WHERE ' . $where . ' ORDER BY created_at DESC, id DESC'; if ( $limit ) { $sql .= ' LIMIT %d OFFSET %d'; array_push( $values, $limit, $offset ); } return (array) $wpdb->get_results( $values ? $wpdb->prepare( $sql, $values ) : $sql ); } // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	private function count( array $filters ): int { global $wpdb; $values = array(); $where = $this->where( $filters, $values ); $sql = 'SELECT COUNT(*) FROM ' . self::table() . ' WHERE ' . $where; return (int) $wpdb->get_var( $values ? $wpdb->prepare( $sql, $values ) : $sql ); } // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	private function distinct( string $column ): array { global $wpdb; if ( ! in_array( $column, array( 'assessment_name', 'profile' ), true ) ) return array(); return array_filter( array_map( 'strval', (array) $wpdb->get_col( 'SELECT DISTINCT ' . $column . ' FROM ' . self::table() . ' WHERE ' . $column . " <> '' ORDER BY " . $column . ' ASC' ) ) ); } // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
