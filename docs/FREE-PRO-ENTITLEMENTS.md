...
AssessCraft resolves the current plan through the `assesscraft_current_plan` filter. The bundled Freemius integration returns `pro` only when premium code is licensed and available. Commercial enforcement is enabled by default.
